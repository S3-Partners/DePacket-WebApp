...
import { type TransactionOptions, type PacketTransaction } from '@/types/tx'
...
type TxActions = {
  addToBatch: (safeTx?: PacketTransaction, origin?: string) => Promise<string>
  signTx: (safeTx?: PacketTransaction, txId?: string, origin?: string) => Promise<string>
  executeTx: (
...
//     const proposeTx = async (sender: string, safeTx: PacketTransaction, txId?: string, origin?: string) => {
...
//     const signRelayedTx = async (safeTx: PacketTransaction, txId?: string): Promise<PacketTransaction> => {
//       assertTx(safeTx)
//       assertWallet(wallet)
...
export const useValidateNonce = (safeTx: PacketTransaction | undefined): boolean => {
  const { safe } = useSafeInfo()
...
export const useSafeTxGas = (safeTx: PacketTransaction | undefined): string | undefined => {
  const { safeAddress, safe } = useSafeInfo()
...
export const useAlreadySigned = (safeTx: PacketTransaction | undefined): boolean => {
  const wallet = useWallet()
  const hasSigned =
