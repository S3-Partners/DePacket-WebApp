...
import type { MetaTransactionData, PacketTransaction } from '@/types/tx'
...
import type { EnvState } from '@/store/slices/settingsSlice'
...
export const getSimulation = async (
  ...
  const data = await fetch(customTenderly!.url, requestObject).then((res) => {
    if (res.ok) {
      return res.json()
    }
