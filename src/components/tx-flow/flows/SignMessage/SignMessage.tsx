...
import { selectBlindSigning } from '@/store/slices/settingsSlice'
...
const SignMessage = ({ message, safeAppId, requestId }: ProposeProps | ConfirmProps): ReactElement => {
  ...
  const { isRiskConfirmed, setIsRiskIgnored } = useContext(TxSecurityContext)
  ...
  const isDisabled = !isOwner || !safe.deployed || !isBlindSigningEnabled
  ...
  const handleSign = async () => {
    ...
    trackEvent({ ...TX_EVENTS.CONFIRM, label: TX_TYPES.typed_message })
  }
  ...
  const onContinue = async () => {
    ...
    //   return
    // }
    // await dispatchPreparedSignature(safeMessage, safeMessageHash, () => setFlow(undefined), requestId)
  }
