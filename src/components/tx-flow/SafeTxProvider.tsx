...
export const SafeTxContext = createContext<{
  ...
  setSafeTx: Dispatch<SetStateAction<PacketTransaction | undefined>>
...
const SafeTxProvider = ({ children }: { children: ReactNode }): ReactElement => {
  const [safeTx, setSafeTx] = useState<PacketTransaction>()
  ...
  const finalSafeTxGas = isSigned
    ? safeTx?.data.packetTxGas
    : (safeTxGas ?? recommendedSafeTxGas ?? safeTx?.data.packetTxGas)

  // Update the tx when the nonce or packetTxGas change
  // useEffect(() => {
  //   if (isSigned || !safeTx?.data) return
  //   if (safeTx.data.nonce === finalNonce && safeTx.data.packetTxGas === finalSafeTxGas) return

  //   createTx({ ...safeTx.data, packetTxGas: String(finalSafeTxGas) }, finalNonce)
  //     .then(setSafeTx)
